...
Подключение модальных окон Micromodal:

Все модальные окна находятся в папке partials в файле popup.html. Файл подключается на страницы через @include('partials/popup.html').
Создаем модальное окно с id. Номер id указываем в кнопке вызова окна в дата-атрибуте. 
Каждое новое окно создаем с индивидуальным номером id и указываем его в дата-атрибуте кнопки.

Пример. Модальное окно с id="modal-1". Кнопка button type="button" data-micromodal-trigger="modal-1">Модальное окно</button

Стили модальных окон находятся в папке scss/vendor/micromodal.css. Для их подключения необходимо расскоментировать строку  @import "./vendor/micromodal";
Для работы модальных окон, необходимо перейти в папку js/_functions.js и раскомментировать функцию вызова модальных окон MicroModal.init();
